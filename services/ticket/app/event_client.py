...
`event`/`event_category`/`ticket_type` now live in event-service's own
`event_svc` Postgres schema, so ticket-service can no longer JOIN into them
directly — these calls replace those JOINs.
"""
