...
`event`/`event_category`/`ticket_type`/`event_permission` now live in
event-service's own `event_svc` Postgres schema, so payment-service can no
longer JOIN into them directly — these calls replace those JOINs.
"""
...
async def get_ticket_types(ticket_type_ids: Iterable[str]) -> dict[str, dict]:
    """Batch fetch — {ticket_type_id: {name, sort_order}}. Ids not found are omitted."""
    ids = sorted({str(i) for i in ticket_type_ids if i})
    if not ids:
        return {}
    async with httpx.AsyncClient(timeout=10) as client:
        resp = await client.get(f"{_BASE}/ticket-types", params={"ids": ",".join(ids)}, headers=_HEADERS)
    resp.raise_for_status()
    return resp.json()
