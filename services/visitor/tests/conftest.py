...
services/visitor/db/init/01_schema.sql's own header comment), so `make test` points
TEST_POSTGRES_* at a second, separately-schemaed test Postgres for this service only."""
import os
import sys
import pathlib
import tempfile
...
_SERVICE_DIR = _HERE.parents[1]        # services/visitor
...
os.environ.setdefault("DB_NAME", os.environ["TEST_POSTGRES_DB"])
os.environ.setdefault("INTERNAL_API_KEY", "test_internal_api_key_min_32_chars_xxxx")
os.environ.setdefault("UPLOADS_DIR", tempfile.mkdtemp(prefix="visitor-test-uploads-"))
...
async def _cleanup(db):
    yield
    await truncate(db, "visitor_photo", "phone_verification", "visitor_log", "visitor_pass", "anonymous_visitor")
